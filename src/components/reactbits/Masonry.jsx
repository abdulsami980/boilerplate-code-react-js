...
const Masonry = ({
  ...
  // Build grid layout with correct heights
  const grid = useMemo(() => {
    if (!width) return [];
    ...
    return items.map((item) => {
      const aspectRatio = item.height / (item.width || columnWidth); // maintain ratio
  ...
  const containerHeight = useMemo(() => {
    ...
    for (let i = 0; i < columns; i++) {
      const colItems = grid.filter((g) => g.x / (g.w + gap) === i);
      const colHeight = colItems.reduce((sum, g) => sum + g.h + gap, 0);
